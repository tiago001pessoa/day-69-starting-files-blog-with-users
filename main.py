...
# print(generate_password_hash("xablau", method="pbkdf2:sha256", salt_length=8))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You have to be logged in to write a comment.")
            return redirect(url_for("login"))
        new_comment = Comment(
            text=request.form["comment"], author=current_user, post=requested_post
        )
